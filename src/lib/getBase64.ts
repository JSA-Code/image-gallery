...
async function getBase64(imageUrl: string) {
  try {
    ...
    if (!res.ok) {
      throw new Error(
        `CANNOT FETCH BLURRED IMAGE! ${res.status} ${res.statusText}`
      );
    }
...
export default async function addBlurredDataUrls(
  ...
  // make all req at once instead of awaiting each one, aka avoiding a "waterfall"
  const base64Promises = images.photos.map((photo) =>
    getBase64(photo.src.large)
  );

  // resolve all reqs in order
